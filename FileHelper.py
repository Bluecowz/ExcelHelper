...
class FileHelper():
    ...
    def archive(settings,excel):
        ...
        shutil.move(settings.my_path + "\\" + settings.active_xl, settings.archive_path + "\\" + new_name)
        shutil.copy(settings.template_path + "\\" + settings.template_name, settings.my_path + "\\" + settings.active_xl)

    @staticmethod
    def create_current(settings):
        """
        In case Current.xlsx does not exist it creates the spreadsheet from the template.
        :param settings: Settings to get files pathes from.
        :return:
        """
        shutil.copy(settings.template_path + "\\" + settings.template_name, settings.my_path + "\\" + settings.active_xl)
